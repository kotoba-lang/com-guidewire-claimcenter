(ns guidewire_claimcenter.main
  "Kotodama WASM entrypoint for the Guidewire_claimcenter clean-room actor (L4) — Clojure port.
  ...
  facts (`guidewire_claimcenter.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "guidewire_claimcenter")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"guidewire_claimcenter.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "guidewire_claimcenter-compat" :tier tier :entities entities} 200])
...
(defn start! [] :guidewire_claimcenter-compat/ready)
